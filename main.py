...
# #DATABASE = '/var/www/html/flaskapp/example.db'
# # DATABASE="D:\\Mohan_Work\\Graduation\\CloudComputing\\Final_Project\\example.db"
# DATABASE = "\\Users\\udaykumarbyroju\\Desktop\\Cincinnati\\Cloud\\Others\\example.db"
...
def connecttoDataBase():
    config = {
    'host':'cloudprojectuday.mysql.database.azure.com',
    'user':'uday',
    'password':'Cloudcomputing@1234',
    'database':'kroger'
    }
...
def readCSVFileandStoretoAzure(csvfilename,dataFrom):
    ...
    df = pd.read_csv("\\Users\\kumarbyroju\\Desktop\\Cincinnati\\Cloud\\Others\\KrogerData\\400_households.csv")
    df.columns = df.columns.str.replace(' ', '')
    df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
    dftotuple = list(df.to_records(index=False))
    for eachtuple in dftotuple:
